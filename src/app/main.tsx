...
function ContextTip(props: {
	...
	return (
		<button className={`context-tip`} onClick={props.onClick}>
			{props.icon}
			{props.title}
			{props.description}
		</button>
...
function ContextTips({ input }: { input: string }) {
	...
	const buttons: ContextTipButton[] = [
		{
			...
			priority:
				lCaseInput.toLowerCase().startsWith("how ") ||
				lCaseInput.toLowerCase().startsWith("what ")
		},
		{
			icon: <></>,
			title: (inp: string) => `Перейти на ${inp}`,
			priority:
				lCaseInput.startsWith("http") ||
				lCaseInput.startsWith("www.") ||
				(!lCaseInput.includes(" ") && lCaseInput.includes(".")),
		...
		},
		{
			...
			title: (inp: string) =>
				`Выполнить ${inp
					...
					.replace("cmd ", "")}`,
			priority:
				lCaseInput.startsWith("run ") ||
				lCaseInput.startsWith("bash ") ||
				lCaseInput.startsWith("cmd ")
		}
	...
	return <div className={"context-tips"}>{elements}</div>;
}
